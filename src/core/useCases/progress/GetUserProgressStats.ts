...
import { CategoryProgress, UserProgressStats } from "@/types/progress";
...
export class GetUserProgressStatsUseCase {
  ...
  async execute(userId: string): Promise<UserProgressStats> {
    ...
    const progressEntries = await this.progressRepository.getUserProgress(
      userId
    );
      ...
      categoryFlashcards.forEach((flashcard) => {
        ...
        if (progress) {
          ...
          if (progress.masteryLevel >= 4) {
            // Poziom 4-5 uznajemy za opanowany
            mastered++;
          } else if (progress.masteryLevel > 0) {
            // Poziom 1-3 uznajemy za w trakcie nauki
            inProgress++;
          }
        }
      });
    ...
    const masteredFlashcards = categoryStats.reduce(
      (sum, cat) => sum + cat.mastered,
      0
    );
    const inProgressFlashcards = categoryStats.reduce(
      (sum, cat) => sum + cat.inProgress,
      0
    );
    const untouchedFlashcards = categoryStats.reduce(
      (sum, cat) => sum + cat.untouched,
      0
    );
    ...
    return {
      ...
      userLevel: 1, // Domyślna wartość
      experiencePoints: 0, // Domyślna wartość
      nextLevelPoints: 100, // Domyślna wartość
      dailyGoal: undefined,
    };
  }
}
