...
interface DemoProgressDashboardProps {
  initialStats: UserProgressStats;
  initialReviewedToday: number;
}

export function DemoProgressDashboard({
  initialStats,
  initialReviewedToday,
}: DemoProgressDashboardProps) {
  const router = useRouter();
  const t = useTranslations('Progress');
  const [stats] = useState<UserProgressStats>(initialStats);
  const [reviewedToday] = useState(initialReviewedToday);
  const [dailyGoal, setDailyGoal] = useState(initialStats.dailyGoal || 5);
  ...
  const handleDailyGoalChange = (value: string) => {
    ...
    // In demo mode, we just update the local state, no server call
  };
      ...
      <main className="container mx-auto p-4 sm:p-6 lg:p-8 bg-black overflow-y-auto">
        ...
        <div className="w-full">
            ...
            {stats.categories.length === 0 ? (
              <div className="bg-black/40 backdrop-blur-md border border-white/10 rounded-xl p-8 text-center">
                <p className="text-gray-400">
                  No demo categories available yet. Start learning to see your progress!
                </p>
                <Button
                ...
                >
                  <CardHeader className="pb-2">
                    <div className="flex justify-between items-center">
                      ...
                      <Link
                        href={`/demo?category=${encodeURIComponent(
                          category.name
                        )}`}
                      >
                        <Button
          ...
          <div className="bg-amber-500/10 border border-amber-500/30 rounded-lg p-4 mb-6">
            <h3 className="text-amber-400 font-semibold mb-2">Demo Mode</h3>
            <p className="text-amber-300/80 text-sm">
              This progress is stored locally in your browser. Sign in to save your real progress across devices and sessions.
            </p>
